...
class RFQAssistant(BaseAgent):
    ...
    async def _analyze_message_context(self, message: str, state: AgentState) -> None:
        ...
        try:
            model = self.llm.with_structured_output(method="json_mode")
            context_result = await model.ainvoke({"messages": [SystemMessage(content=context_prompt)]})
    ...
    async def _extract_rfq_data_with_llm(self, message: str, current_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            model = self.llm.with_structured_output(method="json_mode")
            extraction_result = await model.ainvoke({"messages": [SystemMessage(content=extraction_prompt)]})
