...
from app.dependencies.auth import get_current_user, get_current_supplier, get_current_manufacturer
...
async def get_quotes_for_rfq(
    ...
    try:
        quotes = await rfq_service.get_quotes_for_rfq(rfq_id, current_user.id)
...
async def get_quote(
    ...
    if (quote.supplier_id != current_user.id and 
        quote.rfq.manufacturer_id != current_user.id):
...
async def update_quote(
    ...
    quote_service = QuoteService(db)
    try:
        quote = await quote_service.update_quote(quote_id, current_user.id, update_data)
        if not quote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Quote not found")
        return QuoteResponse.from_orm(quote)
    except ValueError as e:
...
async def submit_quote(
    ...
    quote_service = QuoteService(db)
    try:
        quote = await quote_service.submit_quote(quote_id, current_user.id)
        if not quote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Quote not found")
        return QuoteResponse.from_orm(quote)
    except ValueError as e:
...
async def accept_quote(
    ...
    quote_service = QuoteService(db)
    try:
        quote = await quote_service.accept_quote(quote_id, current_user.id)
        if not quote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Quote not found")
        return QuoteResponse.from_orm(quote)
    except ValueError as e:
